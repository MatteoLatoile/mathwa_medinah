...
const words = {
  fr: {
    ...
    propertyTypes: { apartment: "Appartement", studio: "Studio", villa: "Villa" },
    ...
    photos: {
      viewAll: (n: number) => `Voir les ${n} photos`,
      close: "Fermer",
  ...
  en: {
    ...
    propertyTypes: { apartment: "Apartment", studio: "Studio", villa: "Villa" },
  ...
  ar: {
    ...
    propertyTypes: { apartment: "شقة", studio: "استوديو", villa: "فيلا" },
  ...
  ru: {
    ...
    propertyTypes: { apartment: "Квартира", studio: "Студия", villa: "Вилла" },
    ...
    photos: {
      viewAll: (n: number) => `Показать все фото (${n})`,
      close: "Закрыть",
...
export default async function ListingPage({ params }: PageProps) {
  ...
  const text = words[locale as keyof typeof words] ?? words.fr;
  const rtl = locale === "ar";

  const title =
    locale === "ar"
      ? listing.title_ar || listing.title_fr
      : locale === "en"
        ? listing.title_en || listing.title_fr
        : locale === "ru"
          ? listing.title_ru || listing.title_fr
          : listing.title_fr;

  const description =
    locale === "ar"
      ? listing.description_ar || listing.description_fr
      : locale === "en"
        ? listing.description_en || listing.description_fr
        : locale === "ru"
          ? listing.description_ru || listing.description_fr
  ...
  const monthly = listing.monthly_price !== null;
  const price = monthly ? Number(listing.monthly_price) : Number(listing.yearly_price ?? 0);

  const localeFormat =
    locale === "fr" ? "fr-FR" : locale === "ar" ? "ar-SA" : locale === "ru" ? "ru-RU" : "en-US";

  const propertyTypeLabel = listing.property_type
    ? (text.propertyTypes as Record<string, string>)[listing.property_type] ??
      listing.property_type
    : null;

  // Chaque poste apparaît toujours, coché ou non : c'est ce qui rend
  // la liste utile plutôt qu'une simple vitrine des points forts.
  const includedItems: { label: string; on: boolean; icon: LucideIcon }[] = [
    { label: text.water, on: listing.water_included, icon: Droplet },
    { label: text.electricity, on: listing.electricity_included, icon: Zap },
    { label: text.internet, on: listing.internet_included, icon: Wifi },
    { label: text.cleaning, on: listing.cleaning_included, icon: Sparkles },
  ];

  const equipmentItems: { label: string; on: boolean; icon: LucideIcon }[] = [
    { label: text.airConditioning, on: listing.air_conditioning, icon: Wind },
    { label: text.elevator, on: listing.elevator, icon: ArrowUpDownIcon },
    { label: text.parking, on: listing.parking, icon: Car },
    { label: text.kitchen, on: listing.kitchen, icon: UtensilsCrossed },
  ];

  const infoCells: { icon: LucideIcon; label: string; value: string | number }[] = [
    { icon: Bed, label: text.bedrooms, value: listing.bedrooms },
    { icon: Bath, label: text.bathrooms, value: listing.bathrooms },
    { icon: Armchair, label: text.livingRooms, value: listing.living_rooms },
    { icon: Layers, label: text.floor, value: listing.floor === null ? "—" : listing.floor },
  ];

  if (listing.area_m2 !== null) {
    infoCells.push({ icon: Maximize2, label: text.area, value: `${listing.area_m2} m²` });
  }

  infoCells.push({
    icon: listing.furnished ? Sofa : Package,
    label: text.furnishing,
    value: listing.furnished ? text.furnished : text.unfurnished,
  });

  if (propertyTypeLabel) {
    infoCells.push({ icon: Building2, label: text.propertyType, value: propertyTypeLabel });
  }
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <section className="mt-6">
            <ListingGallery
              ...
              alt={title ?? ""}
              labels={{
                viewAllLabel: text.photos.viewAll(photos.length),
                close: text.photos.close,
                previous: text.photos.previous,
                next: text.photos.next,
              }}
            />
          </section>
              ...
              <Reveal>
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  >
                    ...
                    {listing.verified ? text.verified : text.notVerified}
                  </span>
                ...
                <p className="mt-4 inline-flex items-center gap-1.5 text-base text-ink-soft">
                  ...
                  {listing.location_label || listing.district}
                </p>
              </Reveal>
                ...
                <section className="mt-12">
                  ...
                  <div className="mt-5 flex flex-wrap gap-2.5">
                    {includedItems.map((item) => (
                      <ChecklistPill key={item.label} {...item} />
                    ))}
                  </div>
                </section>
              </Reveal>
            ...
            <aside>
              <Reveal delay={80}>
                <div className="sticky top-28 overflow-hidden rounded-3xl border border-ink/10 bg-paper shadow-[0_12px_40px_rgba(18,33,28,0.06)]">
                  {/* Filigrane discret : la même arche que le logo */}
                  <ArchBullet
                    ...
                    {listing.deposit !== null && (
                      <p className="mt-4 flex items-center gap-2 text-sm text-ink-soft">
                        ...
                        <span className="font-medium text-ink">
                          {"\u20C1"} {Number(listing.deposit).toLocaleString(localeFormat)}
                        </span>
                      </p>
                    )}
...
function InfoCell({
  ...
  return (
    <div className="flex flex-col gap-2.5 bg-paper p-5">
      <Icon className="h-[18px] w-[18px] text-green-700" aria-hidden="true" />
      <div>
...
/** Pastille pour la section "inclus" : vert coché, ou gris barré. Neutre, pas alarmant. */
  ...
  return (
    ...
    >
      ...
      {on ? (
        ...
        <XIcon className="h-3.5 w-3.5 shrink-0 opacity-50" aria-hidden="true" />
      )}
    </span>
  );
}

/** Ligne pour la section "équipements" : même logique, format plus large. */
  ...
  return (
    <div
      className={`flex items-center gap-3 rounded-2xl border px-5 py-4 text-sm ${
        on ? "border-ink/10 bg-paper text-ink" : "border-ink/10 bg-paper/50 text-ink-soft/60"
      }`}
    >
      <Icon
        className={`h-4 w-4 shrink-0 ${on ? "text-green-700" : "text-ink-soft/40"}`}
        aria-hidden="true"
      />
      <span className="flex-1">{label}</span>
      {on ? (
        <CheckIcon className="h-4 w-4 shrink-0 text-green-700" aria-hidden="true" />
      ) : (
        <XIcon className="h-4 w-4 shrink-0 opacity-40" aria-hidden="true" />
      )}
    </div>
  );
}
